...
process ()
{
	cuffdiff -p 4 -L ${sample1},${sample2} -o cuffdiff/${type} /data2/genomes/rn7/refSeqgenes_rn7.gtf $testFiles $ctrlFile
...
do
		...
		sample1=${type}_${test}
		sample2=${type}_${ctrl}
		testFiles=$(ls bam/${sample1}*.UM.sorted.bam | grep -v pooled | awk -v x="" '{x=","$1x}END{sub(",", "", x); print x}')
                ctrlFile=$(ls bam/${sample2}*.UM.sorted.bam | grep -v pooled | awk -v x="" '{x=","$1x}END{sub(",", "", x); print x}')
		process &
      done
done
wait
